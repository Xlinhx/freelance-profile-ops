...
// ============ STABLE MARQUEE MOTION ============
(function() {
  const tracks = [
    { selector: '.marquee-track', speed: 28, hoverSpeed: 9, reduceSpeed: 10 },
    { selector: '.logo-track', speed: 24, hoverSpeed: 8, reduceSpeed: 9 }
  ];
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  function bindMarquee(track, config) {
    if (!track || track.dataset.marqueeBound === 'true') return;
    track.dataset.marqueeBound = 'true';

    const seed = Array.from(track.children).map(node => node.cloneNode(true));
    let cycle = 0;
    let offset = 0;
    let currentSpeed = reduceMotion ? config.reduceSpeed : config.speed;
    let targetSpeed = currentSpeed;
    let last = performance.now();

    function rebuild() {
      if (!seed.length) return;
      const parentWidth = track.parentElement ? track.parentElement.getBoundingClientRect().width : window.innerWidth;
      track.innerHTML = '';
      let guard = 0;
      while ((track.scrollWidth < parentWidth * 2.35 || guard < 2) && guard < 12) {
        seed.forEach(node => track.appendChild(node.cloneNode(true)));
        guard += 1;
      }
      const repeated = Math.max(2, Math.floor(track.children.length / seed.length));
      cycle = Math.max(1, track.scrollWidth / repeated);
      offset = ((offset % cycle) + cycle) % cycle;
      track.style.transform = `translate3d(${-offset}px,0,0)`;
    }

    function setHover(active) {
      targetSpeed = active ? config.hoverSpeed : (reduceMotion ? config.reduceSpeed : config.speed);
      track.classList.toggle('is-hovering', active);
    }

    const shell = track.parentElement || track;
    shell.addEventListener('pointerenter', () => setHover(true));
    shell.addEventListener('pointerleave', () => setHover(false));
    shell.addEventListener('focusin', () => setHover(true));
    shell.addEventListener('focusout', () => setHover(false));

    if ('ResizeObserver' in window) {
      new ResizeObserver(rebuild).observe(shell);
    } else {
    ...
    function frame(now) {
      ...
      if (!document.hidden && cycle > 0) {
        offset = (offset + currentSpeed * (dt / 1000)) % cycle;
        track.style.transform = `translate3d(${-offset}px,0,0)`;
      }
  ...
  function init() {
    tracks.forEach(config => document.querySelectorAll(config.selector).forEach(track => bindMarquee(track, config)));
  }
...
// ============ PROGRESSIVE DETAILS ============
(function() {
  function addDisclosure(card, label, details) {
    if (!card || card.querySelector('.micro-disclosure')) return;
    var panelId = 'detail-' + Math.random().toString(36).slice(2);
    var button = document.createElement('button');
    button.className = 'micro-disclosure';
    button.type = 'button';
    button.setAttribute('aria-expanded', 'false');
    button.setAttribute('aria-controls', panelId);
    button.innerHTML = '<span>' + label + '</span><svg viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg>';

    var panel = document.createElement('div');
    panel.className = 'micro-detail';
    panel.id = panelId;
    panel.innerHTML = '<ul>' + details.map(function(item) { return '<li>' + item + '</li>'; }).join('') + '</ul>';
    ...
    if (tagList) {
      card.insertBefore(button, tagList);
      card.insertBefore(panel, tagList);
    } else {
  ...
  var serviceDetails = [
    ['Discovery use-case trước, không ép AI vào nơi không cần.', 'Thiết kế fallback, logging và human review để chạy production được.', 'Bàn giao prompt/schema/API rõ để team tự vận hành sau.'],
    ['Frontend ưu tiên tốc độ, accessibility và cấu trúc dễ mở rộng.', 'Luồng người dùng được dựng thành prototype trước khi khóa scope.', 'API contract viết theo domain thật, tránh endpoint rời rạc.'],
    ['Data model, transaction và queue được thiết kế theo tải thật.', 'Có logging, error boundary và health-check từ đầu.', 'Tách phần có thể scale khỏi phần chỉ cần đơn giản.'],
    ['Deploy có rollback path, không phụ thuộc thao tác tay.', 'Docker/Nginx/Cloudflare/VPS được ghi lại thành runbook.', 'CI/CD có thể bật/tắt theo môi trường, tránh chạy sai lúc cập nhật docs.'],
    ['Chỉ dùng 3D/realtime khi nó tăng trải nghiệm xem sản phẩm.', 'Kiểm tra canvas, ảnh và motion trên desktop/mobile.', 'Giữ chuyển động nhẹ để không cản đọc nội dung chính.']
  ];
  document.querySelectorAll('.bento-card').forEach(function(card, index) {
    addDisclosure(card, 'Xem cách làm', serviceDetails[index] || serviceDetails[0]);
  });

  var whyDetails = [
    ['Chốt trade-off bằng ngôn ngữ kinh doanh trước khi code.', 'Docs giải thích tại sao, không chỉ liệt kê dùng công nghệ gì.'],
    ['Có update theo mốc, ảnh/demo cụ thể, không báo cáo chung chung.', 'Scope thay đổi được ghi thành quyết định để tránh hiểu lệch.'],
    ['Ưu tiên lát cắt chạy được, sau đó mới mở rộng.', 'Không hy sinh test và deploy path để lấy tốc độ ảo.'],
    ['Bàn giao kèm hướng vận hành, tài khoản, biến môi trường và rủi ro.', 'Hỗ trợ sau go-live để hệ thống sống được trong môi trường thật.'],
    ['Repo, deploy, database và tài liệu thuộc quyền kiểm soát của khách.', 'Không khóa vào service riêng nếu không thật sự cần.'],
    ['AI là một phần của workflow: dữ liệu, kiểm soát lỗi, fallback, đo chất lượng.', 'Luôn phân biệt demo AI với AI có thể vận hành thật.']
  ];
  document.querySelectorAll('.why-card').forEach(function(card, index) {
    addDisclosure(card, 'Mở chi tiết', whyDetails[index] || whyDetails[0]);
  });
})();
...
(function() {
  ...
  document.querySelectorAll('.fx, .fx-stagger, .fx-left, .fx-right, .fx-scale').forEach(el => {
    if (el.dataset.revealed === 'true' || el.classList.contains('in')) return;
    io.observe(el);
  });
...
// ============ CONTEXTUAL SCROLL MOTION ============
(function() {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const layers = [
    { selector: '.about-placeholder-img', strength: 18 },
    { selector: '.faq-cta-img', strength: 12 },
    { selector: '.feedback-card-compact', strength: 10 }
  ];
  const nodes = layers.flatMap(function(layer) {
    return Array.from(document.querySelectorAll(layer.selector)).map(function(el) {
      return { el: el, strength: layer.strength };
    });
  });
  ...
  function update() {
    ...
    nodes.forEach(function(item) {
      ...
      item.el.style.setProperty('--scroll-lift', (-clamped * item.strength).toFixed(2) + 'px');
    });
    ticking = false;
  }
...
(function() {
  ...
  btn.addEventListener('click', function() {
    ...
    if (isOpen) { menu.classList.add('open'); document.body.style.overflow = 'hidden'; }
...
// ============ FLOATING SOCIAL + SCROLL-TO-TOP ============
(function() {
  var social = document.getElementById('floating-social');
  var scrollTop = document.getElementById('scroll-top');
  window.addEventListener('scroll', function() {
    var y = window.scrollY;
    if (social) { if (y > 400) social.classList.add('visible'); else social.classList.remove('visible'); }
    if (scrollTop) { if (y > 500) scrollTop.classList.add('visible'); else scrollTop.classList.remove('visible'); }
  }, { passive: true });
  if (scrollTop) {
...
// ============ PROJECT MODAL (View All) ============
(function() {
  var btn = document.getElementById('btn-view-all');
  ...
  function openModal() {
    // Clone ALL work cards into modal
    grid.innerHTML = '';
    var allCards = document.querySelectorAll('.work-card[data-category]');
    allCards.forEach(function(card) {
      var clone = card.cloneNode(true);
      clone.style.display = '';
      clone.classList.remove('work-extra', 'work-wide', 'work-half');
      // Remove view overlay for modal
      var overlay = clone.querySelector('.view-overlay');
      if (overlay) overlay.remove();
      // Remove arrow
      var arrow = clone.querySelector('.work-arrow');
      if (arrow) arrow.remove();
      // Add click handler for detail modal
      var pid = clone.dataset.project;
      if (pid) {
        clone.addEventListener('click', function(e) {
          e.preventDefault();
          if (window.openProjectDetail) window.openProjectDetail(pid);
        });
      }
...
(function() {
  ...
  var allCards = function() { return document.querySelectorAll('.work-grid > .work-card[data-category]'); };
  ...
  pills.forEach(function(pill) {
    pill.addEventListener('click', function() {
      ...
      allCards().forEach(function(card) {
        var isExtra = card.classList.contains('work-extra');
        if (isExtra) return;
...
var PROJECT_DATA = {
  forge: {
    title: 'Forge — Nền tảng AI Orchestration',
    desc: 'Hệ thống RAG xử lý 2M+ truy vấn mỗi ngày trên 14 phòng ban doanh nghiệp. Pipeline xử lý embedding, vector search, và context-aware response generation cho toàn bộ knowledge base nội bộ.',
    category: 'AI · Production',
    timeline: '4 tháng (Q1–Q2 2025)',
    role: 'Kiến trúc hệ thống & Lead Dev',
    tech: ['Python', 'LangChain', 'PostgreSQL', 'pgvector', 'AWS', 'Docker'],
    highlights: [
      'Xử lý 2M+ queries/ngày với latency < 200ms',
      'RAG pipeline hỗ trợ 14 phòng ban, multi-tenant',
      'Auto-scaling trên AWS ECS + CloudFront CDN',
      'Knowledge ingestion tự động từ Notion, Google Drive, Slack'
    ],
    images: 3
  },
  pulse: {
    title: 'Pulse Analytics — Dashboard Realtime',
    desc: 'Dashboard monitoring realtime cho team vận hành, hiển thị KPIs, alert thông minh và báo cáo tự động. Tối ưu cho việc ra quyết định nhanh dựa trên dữ liệu live.',
    category: 'SaaS · Realtime',
    timeline: '3 tháng (Q3 2025)',
    role: 'Fullstack Developer',
    tech: ['React', 'Node.js', 'WebSocket', 'PostgreSQL', 'Redis', 'Chart.js'],
    highlights: [
      'Realtime data streaming với WebSocket + Redis Pub/Sub',
      'Custom chart components với 60fps animation',
      'Alert engine tự động push notification',
      'Export PDF/Excel báo cáo theo schedule'
    ],
    images: 2
  },
  hienchina: {
    title: 'hienchina.com — Quản lý nhập sỉ/lẻ',
    desc: 'Nền tảng quản lý đơn hàng, tracking shipping, và thanh toán thật cho business nhập hàng Trung Quốc. Xử lý workflow phức tạp từ order → shipping → customs → delivery.',
    category: 'E-commerce',
    timeline: '5 tháng (Q2–Q3 2025)',
    role: 'Fullstack & DevOps',
    tech: ['Next.js', 'NestJS', 'PostgreSQL', 'Docker', 'Nginx', 'VNPay'],
    highlights: [
      'Multi-role: Admin, Nhân viên, Khách hàng',
      'Tích hợp thanh toán VNPay + tracking đơn realtime',
      'Docker Compose production deployment trên VPS',
      'Báo cáo doanh thu, công nợ tự động'
    ],
    images: 3
  },
  conhonan: {
    title: 'conhonannhonbinhdinh.vn — Booking Realtime',
    desc: 'Hệ thống đặt vé sự kiện realtime phục vụ 150-200+ concurrent users. Xử lý race condition, payment verification và seat locking với Redis row-level locking.',
    category: 'Backend · Infra',
    timeline: '2 tháng (Q4 2025)',
    role: 'Backend Architect',
    tech: ['Node.js', 'PostgreSQL', 'Redis', 'PayOS', 'WebSocket', 'SSE'],
    highlights: [
      '150-200+ concurrent users không conflict',
      'Redis row-level locking chống race condition',
      'PayOS integration với webhook verification',
      'Realtime seat availability via SSE'
    ],
    images: 2
  },
  ielts: {
    title: 'mini-ielts.score — AI IELTS Grader',
    desc: 'Chấm bài IELTS Writing/Speaking tự động bằng Gemini API. Phản hồi chi tiết theo band descriptor chính thức của IELTS, gợi ý cải thiện cụ thể.',
    category: 'AI · Education',
    timeline: '6 tuần (Q1 2026)',
    role: 'Fullstack Developer',
    tech: ['Gemini API', 'Node.js', 'React', 'MongoDB'],
    highlights: [
      'Chấm điểm chính xác theo 4 criteria IELTS',
      'Phản hồi chi tiết với gợi ý cải thiện',
      'Audio transcription cho Speaking assessment',
      'History tracking và progress chart'
    ],
    images: 2
  },
  truckcms: {
    title: 'mini-truck-cms — Product Catalog 500+ SKU',
    desc: 'CMS quản lý catalog lớn với watermarking tự động, proxy cache và PLpgSQL stored procedures cho hiệu năng truy vấn cao.',
    category: 'CMS · E-commerce',
    timeline: '2 tháng (Q3 2025)',
    role: 'Backend Developer',
    tech: ['Next.js', 'PostgreSQL', 'PLpgSQL', 'Sharp', 'Nginx'],
    highlights: [
      'Quản lý 500+ SKU với bulk import/export',
      'Auto watermarking ảnh sản phẩm',
      'PLpgSQL stored procedures tối ưu query',
      'CDN proxy cache cho static assets'
    ],
    images: 2
  },
  reconcile: {
    title: 'mini-reconcile — OCR Đối soát Tự động',
    desc: 'OCR và đối soát giao dịch tự động, giảm 90% thao tác nhập liệu thủ công cho kế toán. Xử lý ảnh chứng từ, trích xuất số liệu và matching với database.',
    category: 'AI · Automation',
    timeline: '3 tháng (Q4 2025)',
    role: 'AI/ML Developer',
    tech: ['Python', 'OCR', 'PostgreSQL', 'FastAPI', 'OpenCV'],
    highlights: [
      'Giảm 90% thao tác nhập liệu thủ công',
      'OCR accuracy 98%+ cho chứng từ Việt Nam',
      'Auto-matching transactions với fuzzy logic',
      'Dashboard báo cáo đối soát realtime'
    ],
    images: 2
  },
  student: {
    title: 'mini-student-management-AI — Quản lý học sinh',
    desc: 'Hệ thống quản lý lớp học, điểm danh, báo cáo kết quả tích hợp AI tư vấn học tập cá nhân hóa.',
    category: 'Education · AI',
    timeline: '3 tháng (Q1 2026)',
    role: 'Fullstack Developer',
    tech: ['React', 'NestJS', 'Gemini', 'PostgreSQL'],
    highlights: [
      'AI tư vấn học tập cá nhân hóa cho từng học sinh',
      'Điểm danh QR code + face recognition',
      'Báo cáo kết quả tự động gửi phụ huynh',
...
(function() {
  ...
  function placeholderSVG() {
    return '<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.2"><rect x="4" y="4" width="56" height="56" rx="6"/><circle cx="22" cy="22" r="5"/><path d="M4 48l16-16 8 8 8-8 16 16"/></svg>';
  }
  ...
  window.openProjectDetail = function(projectId) {
    ...
    document.getElementById('detail-timeline').textContent = data.timeline;
    ...
    techEl.innerHTML = data.tech.map(function(t) {
      return '<span class="work-tech-tag">' + t + '</span>';
    }).join('');

    // Highlights
    var hlEl = document.getElementById('detail-highlights');
    hlEl.innerHTML = data.highlights.map(function(h) {
      return '<li>' + h + '</li>';
    }).join('');

    // Gallery — placeholders (user sẽ thay ảnh sau)
    var galleryEl = document.getElementById('detail-gallery');
    var galleryHTML = '';
    for (var i = 0; i < (data.images || 2); i++) {
      galleryHTML += '<div class="detail-gallery-placeholder">' + placeholderSVG() + '<span>Ảnh dự án ' + (i + 1) + ' · Bổ sung sau</span></div>';
    }
  ...
  // Testimonial project-link clicks
  document.querySelectorAll('.testi-project-link').forEach(function(link) {
    link.addEventListener('click', function(e) {
      e.stopPropagation();
      var card = link.closest('[data-link-project]');
      if (card) window.openProjectDetail(card.dataset.linkProject);
    });
  });

  // Testimonial card click also opens linked project
  document.querySelectorAll('.testi-card[data-link-project]').forEach(function(card) {
    card.addEventListener('click', function() {
      window.openProjectDetail(card.dataset.linkProject);
    });
  });
})();
